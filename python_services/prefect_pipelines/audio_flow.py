...
1. Load Audio - Load and validate audio file
...
Features:
- SenseVoice transcription with emotion detection
...
async def summarize_audio_task(transcription_result: TranscriptionResult) -> SummaryResult:
    ...
    The audio_service already generates summaries for long recordings (>2 min),
    ...
    # Get summary from the raw result (audio_service generates it for long audio)
...
async def store_audio_task(
    ...
    # Log storage details (actual MongoDB storage can be added here)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python audio_flow.py <audio_path>")
