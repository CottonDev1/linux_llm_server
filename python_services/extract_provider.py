#!/usr/bin/env python3
"""
Extract schema from EWR.Provider on CHAD-PC
"""
import os
import sys
...
async def main():
    ...
    db_config = DatabaseConfig(
        name='provider',
        server='CHAD-PC',
        database='EWR.Provider',
        lookup_key='ewr_provider',
        user='EWRUser',
        password='66a3904d69',
        port=1433
    )
    ...
    extractor = SchemaExtractor(
        ...
        llm_url='http://localhost:8081',  # General model
    ...
    print(f"LLM: localhost:8081 (general model)", flush=True)
    print(f"{'='*60}\n", flush=True)
