...
/**
 * EWR-DocumentPreviewModal Component
 * A reusable modal for previewing document content with copy/download functionality
 *
 * Usage:
 ...
 * modal.open(title);                   // Open modal and load document by title
 * modal.close();                       // Close the modal
 * modal.getCurrentDocument();          // Get the currently loaded document
 * modal.onDocumentDeleted = (title) => {}; // Callback when document is deleted (admin only)
 */
class EwrDocumentPreviewModal extends HTMLElement {
    constructor() {
        super();
        this._currentDocument = null;
        this.onDocumentDeleted = null; // Callback for when document is deleted
    }

    connectedCallback() {
        const showDelete = this.getAttribute('show-delete') === 'true';
        const id = this.getAttribute('id') || 'documentPreviewModal';

        this.innerHTML = `
            <!-- Document Preview Modal -->
            <div class="ewr-document-popup-overlay" id="${id}-overlay">
                <div class="ewr-document-popup">
                    <div class="ewr-document-popup-header">
                        <h3 class="ewr-document-popup-title" id="${id}-title">Document Preview</h3>
                        <button class="ewr-document-popup-close" id="${id}-close">&times;</button>
                    </div>
                    <div class="ewr-document-popup-body">
                        <div class="ewr-document-popup-meta" id="${id}-meta">
                            <!-- Document metadata will be inserted here -->
                        </div>
                        <div class="ewr-document-popup-content" id="${id}-content">
                            Loading document content...
                        </div>
                    </div>
                    <div class="ewr-document-popup-footer">
                        <div class="ewr-document-popup-footer-left">
                            <button class="ewr-button-action-green-gradient" id="${id}-copy">Copy to Clipboard</button>
                            <button class="ewr-button-action-green-gradient" id="${id}-download">Download</button>
                            ${showDelete ? `<button class="ewr-button ewr-button-medium ewr-button-danger" id="${id}-delete">Delete Document</button>` : ''}
                        </div>
                        <button class="ewr-button-clear" id="${id}-close-btn">Close</button>
                    </div>
                </div>
            </div>
            ...
            <div class="ewr-delete-confirm-overlay" id="${id}-delete-overlay">
                <div class="ewr-delete-confirm-dialog">
                    <h3 class="ewr-delete-confirm-title">Confirm Delete</h3>
                    <p class="ewr-delete-confirm-text" id="${id}-delete-text">Are you sure you want to delete this document? This action cannot be undone.</p>
                    <div class="ewr-delete-confirm-buttons">
                        <button class="ewr-button-clear" id="${id}-delete-cancel">Cancel</button>
                        <button class="ewr-button ewr-button-medium ewr-button-danger" id="${id}-delete-confirm">Delete</button>
                    </div>
                </div>
            </div>
            ` : ''}
    ...
    _bindEvents() {
        ...
        // Close button (X)
        this.querySelector(`#${id}-close`)?.addEventListener('click', () => this.close());

        // Close button (footer)
        this.querySelector(`#${id}-close-btn`)?.addEventListener('click', () => this.close());

        // Copy button
        this.querySelector(`#${id}-copy`)?.addEventListener('click', () => this._copyContent());

        // Download button
        this.querySelector(`#${id}-download`)?.addEventListener('click', () => this._downloadContent());

        // Click outside to close
        this.querySelector(`#${id}-overlay`)?.addEventListener('click', (e) => {
            if (e.target.classList.contains('ewr-document-popup-overlay')) {
                this.close();
            }
        });
        ...
        if (showDelete) {
            // Delete button
            this.querySelector(`#${id}-delete`)?.addEventListener('click', () => this._confirmDelete());

            // Delete cancel
            this.querySelector(`#${id}-delete-cancel`)?.addEventListener('click', () => this._cancelDelete());

            // Delete confirm
            this.querySelector(`#${id}-delete-confirm`)?.addEventListener('click', () => this._executeDelete());

            // Click outside delete modal to close
            this.querySelector(`#${id}-delete-overlay`)?.addEventListener('click', (e) => {
                if (e.target.classList.contains('ewr-delete-confirm-overlay')) {
                    this._cancelDelete();
                }
            });
        }
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                this.close();
                if (showDelete) this._cancelDelete();
            }
        });
    }
    ...
    async open(title) {
        const id = this.getAttribute('id') || 'documentPreviewModal';
        const overlay = this.querySelector(`#${id}-overlay`);
        const titleEl = this.querySelector(`#${id}-title`);
        const metaEl = this.querySelector(`#${id}-meta`);
        const contentEl = this.querySelector(`#${id}-content`);
    ...
    async _copyContent() {
        if (!this._currentDocument || !this._currentDocument.content) {
            alert('No content available to copy');
            return;
        }

        const id = this.getAttribute('id') || 'documentPreviewModal';
        const copyBtn = this.querySelector(`#${id}-copy`);

        try {
            await navigator.clipboard.writeText(this._currentDocument.content);
            ...
            textArea.value = this._currentDocument.content;
    ...
    _downloadContent() {
        ...
        const blob = new Blob([this._currentDocument.content], { type: 'text/plain;charset=utf-8' });
        const url = URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        let filename = this._currentDocument.title || 'document';
    ...
    _confirmDelete() {
        ...
        const id = this.getAttribute('id') || 'documentPreviewModal';
        const deleteOverlay = this.querySelector(`#${id}-delete-overlay`);
        const deleteText = this.querySelector(`#${id}-delete-text`);

        deleteText.innerHTML = `Are you sure you want to delete "<strong>${this._currentDocument.title}</strong>"?<br><br>This will permanently remove the document and all ${this._currentDocument.total_chunks || 1} chunk(s). This action cannot be undone.`;
    ...
    async _executeDelete() {
        ...
        const id = this.getAttribute('id') || 'documentPreviewModal';
        const confirmBtn = this.querySelector(`#${id}-delete-confirm`);
        confirmBtn.disabled = true;
        confirmBtn.textContent = 'Deleting...';

        const documentTitle = this._currentDocument.title;
        ...
        try {
            ...
            if (this.onDocumentDeleted) {
                this.onDocumentDeleted(documentTitle, result.deleted_chunks);
            }
...
customElements.define('ewr-document-preview-modal', EwrDocumentPreviewModal);
